...
function toFsSafeExternalDirName(moduleId) {
    return String(moduleId).replace(/:/g, "_");
}
function nodeSpecifierForShim(moduleId) {
    const s = String(moduleId);
    return s.startsWith("node:") ? s : `node:${s}`;
}
function getNodeExternalModuleId(fromModule, moduleId) {
    return _path.default.relative(_path.default.dirname(fromModule), _path.default.join(METRO_EXTERNALS_FOLDER, toFsSafeExternalDirName(moduleId), "index.js"));
}
...
function tapNodeShimContents(moduleId) {
    return `module.exports = $$require_external('${nodeSpecifierForShim(moduleId)}');`;
}
...
async function tapNodeShims(projectRoot) {
    ...
    for (const moduleId of NODE_STDLIB_MODULES){
        const shimDir = _path.default.join(projectRoot, METRO_EXTERNALS_FOLDER, toFsSafeExternalDirName(moduleId));
